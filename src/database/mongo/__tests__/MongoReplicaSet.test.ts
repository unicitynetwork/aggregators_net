...
async function setupReplicaSet() {
    const containers = await Promise.all([27017, 27018, 27019].map(port => 
    ...
    const uri = 'mongodb://localhost:27017,localhost:27018,localhost:27019/?replicaSet=rs0&serverSelectionTimeoutMS=15000';
...
async function testReplicaFailover() {
    ...
    try {
        ...
        const primaryIndex = containers.findIndex(
