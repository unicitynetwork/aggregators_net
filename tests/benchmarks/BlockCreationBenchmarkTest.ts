...
describe('Block Creation Performance Benchmarks', () => {
  ...
  function createInstrumentedRoundManager(): RoundManager {
    ...
    roundManager.createBlock = async function() {
      ...
      const recordStoragePromise = aggregatorRecords.length > 0 ? 
  ...
  async function formatResult(result: BenchmarkResult): Promise<void> {
    ...
    console.log('\nPhase breakdown:');
    console.log(`- Preparation:  ${result.phases.preparation.toFixed(2)}ms (${(result.phases.preparation * 100 / result.totalTimeMs).toFixed(2)}%)`);
    console.log(`- Submit hash:  ${result.phases.submitHash.toFixed(2)}ms (${(result.phases.submitHash * 100 / result.totalTimeMs).toFixed(2)}%)`);
    console.log(`- Block Finalization: ${result.phases.blockFinalization.toFixed(2)}ms (${(result.phases.blockFinalization * 100 / result.totalTimeMs).toFixed(2)}%)`);
    
    if (result.phases.preparationDetails) {
      const pd = result.phases.preparationDetails;
      console.log('\nPreparation phase details:');
      console.log(`- Get commitments: ${pd.getCommitments.toFixed(2)}ms (${(pd.getCommitments * 100 / result.phases.preparation).toFixed(2)}% of prep)`);
      console.log(`- SMT operations: ${pd.smtOperations.toFixed(2)}ms (${(pd.smtOperations * 100 / result.phases.preparation).toFixed(2)}% of prep)`);
      console.log(`- Record storage: ${pd.recordStorage.toFixed(2)}ms (${(pd.recordStorage * 100 / result.phases.preparation).toFixed(2)}% of prep)`);
    }
