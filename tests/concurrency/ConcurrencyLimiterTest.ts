...
describe('Concurrency Limiter Tests', () => {
  ...
  beforeAll(async () => {
    ...
    gateway.getRoundManager().submitCommitment = async (commitment: Commitment): Promise<boolean> => {
      await delay(500);
      let result = await originalSubmitCommitment(commitment);
  ...
  it('should accept new requests after capacity becomes available', async () => {
    ...
    testLog(`Server load after recovery: ${healthAfterRecovery.activeRequests}/${healthAfterRecovery.maxConcurrentRequests}`);
